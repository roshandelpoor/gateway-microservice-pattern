...
class GatewayController extends AbstractController
{
    ...
    private const SERVICE_URLS = [
        'account' => 'http://gateway-account:80',
        'payment' => 'http://gateway-payment:80', 
        'order' => 'http://gateway-order:80',
    ];
    ...
    public function order(RequestInterface $request, ResponseInterface $response)
    {
        ...
        return $response->withStatus($data->getStatusCode())
            ...
            ->withHeader('Content-Length', strlen($data->getBody()));
    }
    ...
    private function proxyRequest(RequestInterface $request, ResponseInterface $response, string $service): PsrResponseInterface
    {
        ...
        try {
            ...
            return $response->withStatus(502)->withBody(
                new \Hyperf\HttpMessage\Stream\SwooleStream(
                    json_encode([
                        'error' => 'Service unavailable',
                        'message' => 'Unable to connect to ' . $service . ' service',
