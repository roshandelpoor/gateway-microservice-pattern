...
class Redis
{
    ...
    public function __call($name, $arguments)
    {
        ...
        return self::__proxyCall(__CLASS__, __FUNCTION__, ['order' => ['name', 'arguments'], 'keys' => compact(['name', 'arguments']), 'variadic' => ''], function ($name, $arguments) use($__function__, $__method__) {
            ...
            try {
                ...
                $connection->getEventDispatcher()?->dispatch(new Event\CommandExecuted($name, $arguments, $time, $connection, $this->poolName, $result ?? null, $exception ?? null));
                // Release connection.
