...
abstract class AbstractServiceClient
{
    ...
    public function __construct(protected ContainerInterface $container)
    {
        ...
        $this->client = make(Client::class)->setPacker($protocol->getPacker())->setTransporter($transporter)->setNormalizer($protocol->getNormalizer());
    ...
    protected function __generateRpcPath(string $methodName) : string
    {
        ...
        return self::__proxyCall(__CLASS__, __FUNCTION__, ['order' => ['methodName'], 'keys' => compact(['methodName']), 'variadic' => ''], function (string $methodName) use($__function__, $__method__) {
            if (!$this->serviceName) {
    ...
    {
        return $this->dataFormatter->formatRequest(new Request($this->__generateRpcPath($methodName), $params, $id, ['from' => $this->config->get('app_name')]));
    }
    ...
    {
        $nodeArray = $governance->getNodes($address, $this->serviceName, ['protocol' => $this->protocol, 'nodes' => $nodes]);
        $nodes = [];
